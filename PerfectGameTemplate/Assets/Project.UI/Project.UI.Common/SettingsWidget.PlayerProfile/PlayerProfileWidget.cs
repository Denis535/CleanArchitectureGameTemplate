            ...
            view.OnCommand( (PlayerProfileWidgetView.OkeyCommand cmd) => {
                if (!cmd.IsValid) {
                    if (string.IsNullOrWhiteSpace( cmd.Sender.Name.Value )) {
                        var dialog = UILogicalFactory.WarningDialogWidget( "Warning", $"Name is empty" ).OnSubmit( "Ok", null );
                        this.AttachChild( dialog );
                        return;
                    } else {
                        var dialog = UILogicalFactory.WarningDialogWidget( "Warning", $"Name \"{cmd.Sender.Name.Value}\" is invalid" ).OnSubmit( "Ok", null );
                        this.AttachChild( dialog );
                        return;
                    }
                }
